...
const newArr1 = data.map(({ name, hr}) => ({name: name, position: hr.position, salary: hr.salary}));
console.log(newArr1);

//2. Ստեղծել աշխատակիցների ցուցակի նոր զանգված որոնց աշխատավարձը >=150000;

const newArr2 = data.filter((el) => {return Number(el.hr.salary.replace(/\D/g, '')) >= 150000});
console.log(newArr2);

//3. Գտնել ամենից վաղ աշխատակցին; ( new Date() )

let minDate = new Date(data[0].hr.start_date); 
let employee = data[0];
for(let x in data) {
    let curDate = new Date(data[x].hr.start_date);
    if (curDate < minDate) {
        minDate = curDate;
        employee = data[x];
    }
...
//4. Ստանալ այն աշխատակիցների ցուցակը, որոնց աշխատավարձը <= 100000 և ավելացնել նոր դաշտ՝
// բոնուս, որի արժեքը = աշխատավարձի 20%;

const newArr3 = data.filter((el) => {return Number(el.hr.salary.replace(/\D/g, '')) <= 100000});
for(let x in newArr3) {
    let bonus = newArr3[x].hr.salary.replace(/\D/g, '') * 0.2;
    newArr3[x].hr.bonus = '$' + bonus;    
}
console.log(newArr3);

//5. Ստանալ աշխատակիցների քանակը ըստ քաղաքների

const numberEmployeesByCity = (data) => data.reduce((acc, el) => ((acc[el.contact[0]] = acc[el.contact[0]] ? acc[el.contact[0]] + 1 : 1), acc), {});
console.log(numberEmployeesByCity(data));
...
const employees = data.map(obj => obj.name);  
setTimeout(() => { console.log('Մեր բոլոր աշխատակիցները։', employees); }, 3000);

const index = Math.floor(Math.random() * employees.length);
const randomEmployee = employees[index];
setTimeout(() => { console.log('Ներող ապեր քեզ դուրս ենք հանում:', randomEmployee); }, 4000);

setTimeout(() => { employees.splice(index, 1); }, 4000);
setTimeout(() => { console.log('Նոր աշխատակիցների ցուցակ։', employees); }, 4000);
